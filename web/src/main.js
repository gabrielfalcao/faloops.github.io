...
function randomInterval() {
    return (Math.floor(Math.random() * (faloopsConfig.maxInterval - faloopsConfig.minInterval + 1)) + faloopsConfig.minInterval);
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    ["min", "max"].forEach(kind => {
        // @ts-ignore: TS2740
        const input = document.getElementById(`freq-${kind}`);
